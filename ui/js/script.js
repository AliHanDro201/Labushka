...
document.addEventListener("DOMContentLoaded", function() {
    // Получаем элементы переключателя и иконки
    const toggleButton = document.getElementById("toggleButton");
    const micIcon = document.getElementById("micIcon");
    ...
    if (toggleButton) {
        ...
        toggleButton.addEventListener("change", function() {
            if (!toggleButton.checked) {
                // Когда переключатель выключен (unchecked) – останавливаем аудио
                eel.stop_audio_ui()(function(response) {
                    console.log("Ответ от stop_audio_ui:", response);
                    // Например, можно поменять класс иконки или её стиль
                    micIcon.classList.remove("fa-microphone-lines");
                    micIcon.classList.add("fa-microphone-lines-slash");
                });
            } else {
                // Дополнительно: если необходимо включать аудио, можно запустить функцию запуска записи/озвучки
                console.log("Переключатель включён – аудио работает");
                // Здесь можно добавить вызов, который запускает озвучку или запись, если это требуется.
                micIcon.classList.remove("fa-microphone-lines-slash");
                micIcon.classList.add("fa-microphone-lines");
            }
        });
    } else {
        console.log("Элемент с id 'toggleButton' не найден.");
    }
});
...
function processUserInput(userText) {
    ...
    function onUserInput(text) {
        if (text.toLowerCase().startsWith(trigger)) {
            ...
            if (query) {
                // Вызываем функцию process_input, которая связана с Python через Eel
                eel.process_input(query)(function(response) {
                    addMessageToChat("Система (браузер): " + response);
                });
            } else {
                addMessageToChat("Ошибка: не указан запрос после 'обратись к gpt'");
            }
        } else {
            // Для остальных сообщений вызываем стандартную обработку
            eel.generate_gpt_response(text)(function(response) {
                addMessageToChat("Система: " + response);
            });
        }
    }
}
